...
class Fecha extends CI_Controller {
	...
	public function __construct() {
		...
		$this -> searchfields[1]='fecha';
		$this -> searchfields[2]='descripcion';
		
		$this -> load -> library('session');
		$this -> name = "fechaid";
		$this -> searchname = "fechasearch";
	}
	public function combo() {
		...
		foreach ($fechas as $key => $value) {
			$answer[] = array("id" => $value['id'], "fecha" => $value['fecha']);
		}
	...
	public function all() {
		...
		if (isset($_SESSION[$this -> searchname]) == true && strlen($_SESSION[$this -> searchname]) > 0) {
			...
			foreach ($this->searchfields as $i => $j) {
				...
				$fecha_info = $this -> model_fecha -> like($data);
				if(isset($fecha_info['data'])==true){
					foreach ($fecha_info['data'] as $index => $info) {
						if(isset($info['id'])==true){
							$fecha_data[$info['id']] = $info;
						}
					}	
				}				
			}
	...
	public function update() {
		...
		if ($flag) {
			$this -> model_fecha -> update($id, $data);
			$answer = array("success" => "true");
		}
	...
	public function data(){
		...
		$data=$this -> model_fecha -> getOne($id);
...
/* Location: ./application/controllers/fecha.php */
?>
